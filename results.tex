...
\subsection{Causal Evidence: Multi-Outcome DiD}
\label{sec:res_causal}

\subsubsection{Which margins respond to governor imposition?}

The TWFE DiD (Figure~\ref{fig:did_coef}; Table~\ref{tab:did_results}) reveals a striking pattern: safety margins respond causally, while demand and dynamics margins do not.
...
This pattern is consistent with the cross-sectional evidence: governance affects acceleration aggressiveness (a safety margin) but not riding variability, cruising patterns, or demand (dynamics and demand margins).
...
\subsection{No Behavioral Compensation on Any Margin}
\label{sec:res_compensation}

\subsubsection{Aggregate vs.\ within-user effects}
...
\subsubsection{Placebo validation}
