...
The TUB-mediated fraction of the experience effect is quantified via a decomposition. Let $\bar{Y}^{(j)}_b$ denote the mean of outcome $j$ for trips in experience bin $b$ across all modes, and $\bar{Y}^{(j)}_{b,\text{STD/ECO}}$ the corresponding mean restricted to STD/ECO trips. The total experience effect is $\Delta^{(j)}_{\text{total}} = \bar{Y}^{(j)}_{B} - \bar{Y}^{(j)}_{1}$ (bin $B$ = trips 101--200 vs.\ bin 1), and the within-governed component is $\Delta^{(j)}_{\text{gov}} = \bar{Y}^{(j)}_{B,\text{STD/ECO}} - \bar{Y}^{(j)}_{1,\text{STD/ECO}}$. The mediation percentage is $M^{(j)} = 1 - \Delta^{(j)}_{\text{gov}} / \Delta^{(j)}_{\text{total}}$. Confidence intervals are obtained by bootstrapping users with replacement (500 iterations, $n = 50{,}000$ users per iteration) and computing the 2.5th and 97.5th percentiles of $M^{(j)}$.
