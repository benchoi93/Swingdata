...
\multicolumn{5}{l}{\footnotesize Demand outcomes (trip count, active users) are city-month aggregates, analyzed via DiD only (Table~\ref{tab:did_results}).} \\
\end{tabular}
\end{table}

\subsubsection{Individual fixed effects vs.\ GEE}

Table~\ref{tab:fe_vs_gee} presents individual FE estimates alongside the GEE results. All 12 mode--outcome comparisons maintain the same sign, confirming that mode effects are robust to controlling for time-invariant user heterogeneity. However, the magnitude of attenuation reveals an important pattern.

Safety outcomes attenuate by 15--22\% under FE: harsh acceleration STD changes from $-0.61$ (GEE) to $-0.52$ (FE), and harsh deceleration STD from $-0.63$ to $-0.49$. This indicates that GEE overestimates safety effects because risk-seeking riders disproportionately select into TUB---a between-user confound that FE absorbs. Despite attenuation, all safety mode effects remain highly significant ($p < 0.001$).

Dynamics outcomes show the opposite pattern: FE estimates are substantially \textit{larger} than GEE. Speed CV amplifies from $-0.022$ (GEE) to $-0.089$ (FE), and cruise fraction from $+0.061$ to $+0.112$. The GEE underestimates within-user mode effects on riding dynamics because between-user heterogeneity (e.g., inherently erratic vs.\ smooth riders choosing different modes) masks the within-user governance effect.

The divergence between GEE and FE confirms endogeneity from mode self-selection: the Hausman diagnostic favors FE for causal interpretation of cross-sectional mode effects. The key substantive conclusion is unchanged: governance reduces acceleration aggressiveness (the primary safety concern) even after absorbing all time-invariant rider characteristics.

\begin{table}[htbp]
\centering
\caption{GEE vs.\ individual fixed effects: mode coefficients (TUB = reference).}
\label{tab:fe_vs_gee}
\small
\begin{tabular}{lrrrrl}
\toprule
\textbf{Outcome} & \textbf{Mode} & \textbf{GEE} & \textbf{FE} & \textbf{$\Delta$\%} & \textbf{Direction} \\
\midrule
\multicolumn{6}{l}{\textit{Safety (Poisson)}} \\
Harsh accel & STD & $-0.610$ & $-0.520$ & $-14.8$ & Attenuated \\
 & ECO & $-2.863$ & $-2.444$ & $-14.6$ & Attenuated \\
Harsh decel & STD & $-0.627$ & $-0.490$ & $-21.8$ & Attenuated \\
 & ECO & $-3.269$ & $-2.791$ & $-14.6$ & Attenuated \\
\addlinespace
\multicolumn{6}{l}{\textit{Safety (OLS/Gaussian)}} \\
Speed CV & STD & $-0.022$ & $-0.089$ & $+301$ & Amplified \\
 & ECO & $-0.039$ & $-0.130$ & $+234$ & Amplified \\
\addlinespace
\multicolumn{6}{l}{\textit{Dynamics (OLS/Gaussian)}} \\
Cruise fraction & STD & $+0.061$ & $+0.112$ & $+82$ & Amplified \\
 & ECO & $+0.160$ & $+0.238$ & $+49$ & Amplified \\
Zero-speed frac. & STD & $-0.008$ & $-0.041$ & $+416$ & Amplified \\
 & ECO & $-0.007$ & $-0.047$ & $+586$ & Amplified \\
\addlinespace
\multicolumn{6}{l}{\textit{Manipulation check (OLS/Gaussian)}} \\
Mean speed & STD & $-1.942$ & $-0.855$ & $-56$ & Attenuated \\
 & ECO & $-4.221$ & $-2.741$ & $-35$ & Attenuated \\
\bottomrule
\multicolumn{6}{l}{\footnotesize $\Delta$\%: percentage change from GEE to FE. Negative = FE smaller (attenuation); positive = FE larger.} \\
\multicolumn{6}{l}{\footnotesize GEE: 200K subsample, exchangeable correlation. FE: 1.8M trips, user-clustered SEs. All $p < 0.001$.} \\
\end{tabular}
\end{table}
...
P4 & Cruise fraction & Moderate GEE effect ($\beta = +0.06$); large paired $d$ is speed-mediated & No significant causal change \\
...
TUB adoption increases monotonically with experience: from 23.0\% of trips at trip~1 to 69.5\% at trips~101--200 (Figure~\ref{fig:experience}). The adoption curve is concave, rising steeply during the first 20--30 trips before plateauing. This pattern suggests a learning-and-selection process: new riders initially use the default governed mode, discover TUB (likely through peer influence or in-app exploration), and progressively shift their mode preferences.

This adoption trajectory mediates the experience-outcome gradient differently across outcome categories, revealing the mechanism behind the experience-risk gradient:

Safety outcomes are heavily mediated by TUB adoption. Bootstrap confidence intervals (500 iterations, user-level resampling) confirm the mediation estimates are precise: harsh acceleration shows 70.0\% mediation (95\% CI: [65.3, 74.8]), mean speed 63.2\% [58.2, 66.0], and harsh deceleration 57.0\% [53.4, 60.4]. In contrast, dynamics outcomes are less mediated: speed CV 26.0\% [22.7, 29.4], cruise fraction 20.6\% [18.5, 22.8], zero-speed fraction 18.4\% [15.5, 20.8]. All CIs exclude zero, confirming that TUB adoption mediates the experience-outcome gradient across all margins, but the safety--dynamics gap is robust: the lower bound of the harsh acceleration CI (65.3\%) exceeds the upper bound of the highest dynamics outcome (speed CV, 29.4\%). The differential mediation rates are consistent with the cross-sectional SHAP analysis (Section~\ref{sec:res_prediction}): outcomes for which mode is the dominant predictor are highly mediated, while outcomes driven by trip characteristics or traffic conditions are not.

The implication is that experienced riders' higher harsh event rates are primarily a mode selection story---they choose TUB more---not an inherent change in riding aggressiveness within a given mode. To verify this, we compute within-mode experience gradients: among STD-only trips, the difference in harsh acceleration between trip~1 and trip~100+ is only 0.04 events per trip, compared to 0.18 events per trip in the pooled sample. The experience-risk gradient is 78\% attenuated once mode selection is controlled, confirming mediation as the primary pathway.
...
First speeding & 66.1\% & 7 / $\infty$ trips & 60,963 & 8.13$^\ddagger$ \\
\bottomrule
\multicolumn{5}{l}{\footnotesize $^*$Protective: TUB users take longer to reach high-CV events (mechanical, not behavioral).} \\
\multicolumn{5}{l}{\footnotesize $^\ddagger$HR for tub\_frac (continuous); binary ever\_tub HR $= 5.73$ ($C = 0.819$), reported in text.} \\
\multicolumn{5}{l}{\footnotesize $\infty$ = median not reached (>50\% of non-TUB users never speed). All log-rank $p < 0.001$.} \\
\multicolumn{5}{l}{\footnotesize Schoenfeld residual tests indicate PH violations for tub\_frac in all three models ($p < 0.001$).} \\
\multicolumn{5}{l}{\footnotesize Schoenfeld--time correlations are negative (tub\_frac: $\rho = -0.05$ to $-0.15$), indicating HRs attenuate} \\
\multicolumn{5}{l}{\footnotesize over the trip sequence. HRs represent average effects (see Section~\ref{sec:disc_limitations}).} \\
\end{tabular}
\end{table}
